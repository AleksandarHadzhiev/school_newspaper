...
function EditArticle(props){
    ...
    const article = {
        "personId":props.article.personId,
    ...
    function createArticle(){
        ...
        if(message===""){
            ArticleServices.editArticle(article,token).then((res)=>{
                    if(res.data===true){
                        window.location="/";
                    }
            ...
            <div className={classes.content}>
                <form className={classes.form}>
                    <div className={classes.element}>
                        <label>Title:</label>
                        <input defaultValue={props.article.title} type="text" onChange={(e)=>{setAticleTitle(e.target.value)}} placeholder="Input title here...."/>
                    </div>
                    <div className={classes.element}>
                        <label>Content:</label>
                        <textarea defaultValue={props.article.textContent} onChange={(e)=>{setArticleContent(e.target.value)}} placeholder="Input content here...." rows="15" cols="50"/>
                    </div>
